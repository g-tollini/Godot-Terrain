...
const source_fragment = "
		...
		void main() {
			...
			{
				vec2 uv0 = mul0 * vec2(-uv.x, uv.y); // using different flipped uvs like vec2(-uv.x, uv.y) aims at countering the effect of uneven low frequency terrain not producing high frequency random visuals when sampled
				vec2 uv1 = mul0 * vec2(uv.x, -uv.y);
				vec2 uv2 = mul1 * vec2(uv.x, -uv.y);
				
				vec2 mirrored_uv0 = vec2(1) - abs(mod(uv0, 2.0) - 1.0); // using mirrored (pingpong) uvs because the fbm texture does not repeat
				vec2 mirrored_uv1 = vec2(1) - abs(mod(uv1, 2.0) - 1.0);
				vec2 mirrored_uv2 = vec2(1) - abs(mod(uv2, 2.0) - 1.0);
				
				n = 2 * (texture(heightmap, uv).xyz - vec3(0.5)) +
				  1 / (mul2) * 2 * (texture(heightmap, mirrored_uv0).xyz - vec3(0.5)) // no sqrt here because it looked nicer without it
				+ 1 / (mul2) * 2 * (texture(heightmap, mirrored_uv1).xyz - vec3(0.5))
				+ 1 / sqrt(mul1) * 2 * (texture(heightmap, mirrored_uv2).xyz - vec3(0.5)); // we actually increase the amplitude of the sample with the frequency, we should be doing the opposite but this gives better results imo
			}
			else
			{
				n = fbm(noise_pos.xz);
